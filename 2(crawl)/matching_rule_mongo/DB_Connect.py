...
class task_opt():
    ...
    def select_crawl_task(self):#最多获取6个任务
        Sql_info = 'select top 6 Task_URL,Task_On_rule,Task_Out_rule,Task_Deep,Task_Match_field,Task_Match_rule,Task_isClear,RID from Crawl_Task WHERE (Task_Status=0) ORDER BY Task_Level ASC ;'  # 根据任务ID得到具体任务信息
    ...
    def init_crawl_task(self):#脚本运行前动作，将中间表清空，将运行状态的任务和就绪态的任务更改为初始态
        ...
        self.db_obj.WriteSql(Sql_info)
        sql = "update Crawl_Task set Task_Status= 0  WHERE Task_Status=1 or Task_Status=2(crawl);"#将就绪态和执行态值为初始态
        self.db_obj.WriteSql(sql)
    ...
    def test(self):
        sql = "select COUNT(RID) from Crawl_Task_Rule;"
        result = self.db_obj.ReadSql(sql)
        return result

    def test1(self):
        Sql_info = 'select Task_URL,Task_On_rule,Task_Out_rule,Task_Deep,Task_Match_field,Task_Match_rule,Task_isClear,RID from Crawl_Task WHERE (RID=219);'  # 根据任务ID得到具体任务信息
        result = self.db_obj.ReadSql(Sql_info)
        return result
