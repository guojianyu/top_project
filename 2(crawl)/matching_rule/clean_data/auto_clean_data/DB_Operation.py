...
class collection_db:
    def __init__(self):
        conn = MongoClient('192.168.5.10', 27000,connect=False)
    ...
    def delete_repeat_data(self,_id,Article_Keyword,flag=2):#删除数据库中的重复文章数据
        #return self.tb.remove({"_id":_id})
        print (_id)
        return self.tb.update({"_id":_id},{"$set":{"flag":flag,"Article_Keyword":Article_Keyword}})
    ...
    def test(self):
         return self.tb.find_and_modify(query={"flag":0},
                                         update={"$set":{"flag":1}})
    def test1(self,data):
        self.tt.insert(data)
