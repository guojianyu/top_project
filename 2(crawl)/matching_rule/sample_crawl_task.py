...
if __name__ =='__main__':
    RID = 1#自定义任务ID
    input_url = ""#主站url  必填
    Task_Deep = 10#爬取深度 必填且必须时数字
    #需要过滤就填不需要过滤就默认
    Task_On_rule='["1:shipin/","2:php"]'#域名必须包含   1：域名 2：目录 3：文件
    Task_Out_rule = '["1:shipin/","2:php"]'#域名必须不包含 1：域名 2：目录 3：文件
    Task_Match_field = '["title", "keywords", "description", "path", "content"]'
    Task_Match_rule = '[["<title>","</title>"],["<meta.*?keywords.*?content=",">","<meta content=","name=.*?keywords"],["<meta.*?description.*?content=",">","<meta content=","name=.*?description"],["<b>当前位置：","</div>","<b>当前位置","</div>","您的位置是","</div>"],["<div class=\\"newsDetailSummary3\\">","<div class=\\"newsDetailPage\\">","<div class=\\"newsDetailSummary3\\">","<div class=\\"newsDetailRight\\">","<div class=\\"newsDetailMessages.*?>","<div class=","<div class=\\"luncenzhongxin3_listBar\\">","<div class=\\"luncenzhongxin3_listBar"]]'

    task = {"Task_isClear":0,"Task_URL":input_url,"Task_Deep":Task_Deep,"Task_On_rule":Task_On_rule,"Task_Out_rule":Task_Out_rule,"Task_Match_field":Task_Match_field,"Task_Match_rule":Task_Match_rule,"RID":RID}
    obj = crawl_obj()
    task = [task["Task_URL"],task["Task_On_rule"],task["Task_Out_rule"],task["Task_Deep"],task["Task_Match_field"],task["Task_isClear"],task["RID"]]
    obj.run(task)
