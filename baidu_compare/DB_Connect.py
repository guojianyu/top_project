...
class task_opt():
    ...
    def insert_Article_List_Summary(self,summary,id):
        sql = "insert into Article_Summary_Tags(Article_RID,Article_Summary) VALUES (%r,%r);" % (id,(summary.replace("'",'"')))
        self.db_obj.WriteSql(sql)

    def look_biao(self):
        Sql_info = "select  top 1*  from Article_List WHERE (isCompare=1 or isCompare=2(crawl));"  # 根据任务ID得到具体任务信息
        result = self.db_obj.ReadSql(Sql_info)
        return result
class SqlRW(object):
