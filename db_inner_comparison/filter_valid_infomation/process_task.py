...
class filter:
    ...
    def __init__(self):
        ...
        workbook = xlrd.open_workbook(r'C:\untitled\db_inner_comparison\filter_valid_infomation\匹配词.xlsx')
    ...
    def filter_valid_info(self,data_list):  # 过滤有效信息
        for item in data_list:
            #print('标题:', item[1])
            ci = self.cut_word(item[1])
            #print('词频：', ci)
            if not ci:
                continue
            self.record_word_excel(ci, item[1],item[0])
        pass

    def record_word_excel(self,words_list, title,rid):
        find_flag = 0
        for word in words_list:
            if word in self.col_content:
                find_flag = 1
                break
        if not find_flag:
